...
// https://forum.arduino.cc/t/default-callback-function-in-a-library/517808
...
class RotaryEncoder
{
  ...
  public:
    RotaryEncoder(const CallbackFunc RotaryCallback, const byte DT_Pin = 4, const byte CLK_Pin = 5, const byte SW_Pin = 3) : RotaryCallback(RotaryCallback), DT_Pin(DT_Pin), CLK_Pin(CLK_Pin), SW_Pin(SW_Pin)
    {
